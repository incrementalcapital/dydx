...
# Determine most competitive bid limit price
...
# Set bid amount to the minimum allowable trade size (no associated fees)
quantity = consts.SMALL_TRADE_SIZE_WETH / (10 ** consts.DECIMALS_WETH)

try:
    # Submit order to dYdX
    submission = postbid( orderinginfo[3].quantize( Decimal( quotetick ) ), quantity )
...
logger.info ( f'Buying {quantity} ETH at {orderinginfo[3].quantize( Decimal( quotetick ) )} DAI/ETH.' )
